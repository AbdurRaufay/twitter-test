...
const Twitter = () => {
  ...
  const handleTwitterLogin = async () => {
    try {
      ...
      const popup = window.open('https://localhost:8080/auth/twitter', '_blank', 'width=600,height=400');
      if (popup) {
        // Listen for messages from the pop-up window
        window.addEventListener('message', (event) => {
          if (event.origin === 'https://localhost:8080' && event.data === 'twitter-auth-success') {
